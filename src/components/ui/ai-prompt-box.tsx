...
import { ArrowUp, Square, X, Mic, BrainCog } from "lucide-react";
...
export const PromptInputBox = React.forwardRef((props: PromptInputBoxProps, ref: React.Ref<HTMLDivElement>) => {
  const { onSend = () => {}, isLoading = false, placeholder = "Ask about backtesting...", className } = props;
    ...
    >
      ...
      <PromptInputActions className="flex items-center justify-between gap-2 p-0 pt-2">
        <div className="flex items-center gap-1 transition-opacity duration-300 opacity-100 visible">
          <PromptInputAction tooltip="Backtest parameters">
            <button
              className="flex h-8 w-8 text-[#9CA3AF] cursor-pointer items-center justify-center rounded-full transition-colors hover:bg-gray-600/30 hover:text-[#D1D5DB]"
              disabled={isLoading}
            >
              <BrainCog className="h-5 w-5 transition-colors" />
            </button>
          </PromptInputAction>
        </div>

        <PromptInputAction tooltip={isLoading ? "Stop" : hasContent ? "Send" : "Send query"}>
          <Button
            variant="default"
            size="icon"
            className={cn(
