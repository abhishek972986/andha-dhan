...
import {
  Plus, Lightbulb, Paperclip, ImageIcon, FileCode,
  ChevronDown, Check, Sparkles, Zap, Brain, Bolt, Github, LineChart,
  SendHorizontal
} from 'lucide-react'
            ...
            <div className="p-1.5">
              ...
              {models.map((model) => (
                ...
                >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {model.badge && (
                        <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${
                          model.badge === 'Pro' ? 'bg-indigo-500/20 text-indigo-300' : 'bg-blue-500/20 text-blue-300'
                        }`}>
                          {model.badge}
                        </span>
                      )}
                  ...
                  {selected.id === model.id && <Check className="size-4 text-blue-400 shrink-0" />}
                </button>
              ))}
            </div>
          </div>
      ...
      <div className="relative rounded-2xl bg-[#1e1e22] ring-1 ring-white/[0.08] shadow-[0_0_0_1px_rgba(255,255,255,0.05),0_2px_20px_rgba(0,0,0,0.4)]">
        ...
        <div className="flex items-center justify-between px-3 pb-3 pt-1">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <span className="hidden sm:inline">{isLoading ? 'Sending...' : actionLabel}</span>
...
function AnnouncementBadge({ text, href = '#' }: { text: string; href?: string }) {
  const content = (
    <>
      ...
      <span className="absolute -top-px left-1/2 -translate-x-1/2 h-[2px] w-[100px] opacity-60" style={{ background: 'linear-gradient(90deg, transparent 0%, rgba(37, 119, 255, 0.8) 20%, rgba(59, 130, 246, 0.8) 50%, rgba(37, 119, 255, 0.8) 80%, transparent 100%)', filter: 'blur(0.5px)' }} />
  ...
  return (
    <div className="flex items-center gap-4 justify-center">
      ...
      <div className="flex gap-2">
        {[
          { id: 'tradingview', name: 'TradingView', icon: <LineChart className="size-4" /> },
...
export function BoltStyleChat({
  ...
  return (
    <div className="relative flex flex-col items-center justify-center min-h-screen w-full overflow-hidden bg-[#0f0f0f]">
      <RayBackground />
      <div className="absolute top-[58px] z-10">
        <AnnouncementBadge text={announcementText} href={announcementHref} />
      </div>

      <div className="relative z-10 flex flex-col items-center justify-center w-full min-h-screen overflow-hidden px-4 pt-28 pb-10">
