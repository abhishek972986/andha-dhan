...
const triggerVariants = cva(
  ["group inline-flex items-center justify-between gap-2 outline-none cursor-pointer",
   "text-[13px] h-9 px-3", "transition-all duration-80",
   "disabled:opacity-50 disabled:pointer-events-none", "focus-visible:ring-1 focus-visible:ring-blue-500"],
  {
    variants: {
      variant: {
        bordered: "border border-white/25 bg-white/10 text-white hover:bg-white/15",
        borderless: "border border-transparent bg-transparent text-white hover:bg-white/10",
      },
    },
      ...
      <div className="flex flex-col gap-1">
        ...
        >
          <span className="flex items-center gap-2 min-w-0 flex-1">
            {Icon && <Icon size={16} strokeWidth={1.5} className="shrink-0 text-white/70 transition-[color,stroke-width] duration-80 group-hover:text-white group-hover:stroke-[2]" />}
            <span className="min-w-0 flex-1 text-left truncate text-sm">
              {label ?? <span className="text-white/50">{placeholder}</span>}
            </span>
          </span>
          <svg width={14} height={14} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="shrink-0 text-white/50 transition-colors duration-80 group-hover:text-white/70">
            <path d="M6 9l6 6 6-6" />
          </svg>
        </button>
  ...
  ({ className, children }, ref) => {
    ...
    return createPortal(
      <SelectContentContext.Provider value={{ registerItem, activeIndex, checkedIndex }}>
        <div style={{ position: "fixed", top: triggerRect.bottom + 8, left: triggerRect.left, minWidth: triggerRect.width, zIndex: 50 }}>
          <motion.div
            ref={(node) => {
            ...
            className={cn(`relative flex flex-col gap-0.5 max-h-[300px] overflow-y-auto ${shape.container} bg-neutral-900/80 backdrop-blur-md shadow-[0_8px_32px_rgba(0,0,0,0.8)] border border-white/20 p-1 select-none outline-none`, className)}
          ...
          >
            <AnimatePresence>
              {checkedRect && (
                <motion.div className={`absolute ${shape.bg} bg-white/10 dark:bg-white/5 pointer-events-none`} initial={false}
                  animate={{ top: checkedRect.top, left: checkedRect.left, width: checkedRect.width, height: checkedRect.height, opacity: isHoveringOther ? 0.5 : 1 }}
                  exit={{ opacity: 0, transition: { duration: 0.12 } }} transition={{ ...springs.moderate, opacity: { duration: 0.08 } }} />
              )}
            </AnimatePresence>
            <AnimatePresence>
              {activeRect && (
                <motion.div key={sessionRef.current} className={`absolute ${shape.bg} bg-white/15 dark:bg-white/10 pointer-events-none`}
            ...
            <AnimatePresence>
              {focusRect && (
                <motion.div className={`absolute ${shape.focusRing} pointer-events-none z-20 border border-blue-500`} initial={false}
  ...
  ({ className, children, icon: Icon, value, index, disabled = false, ...props }, ref) => {
    ...
    return (
      <div
        ...
        className={cn(`relative z-10 flex items-center gap-2 ${shape.item} px-2 py-2 text-[13px] cursor-pointer outline-none select-none`, "transition-[color] duration-80", isActive || isChecked ? "text-white" : "text-white/60", disabled && "opacity-50 pointer-events-none", className)}
        {...props}
      >
        {Icon && <Icon size={16} strokeWidth={isActive || isChecked ? 2 : 1.5} className="shrink-0 transition-[color,stroke-width] duration-80" />}
        <span className="flex-1 min-w-0 truncate text-sm">{children}</span>
        <AnimatePresence>
          {isChecked && (
            <motion.svg key="check" width={16} height={16} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="shrink-0 text-white" initial={{ opacity: 1 }} animate={{ opacity: 1 }} exit={{ opacity: 1 }}>
...
const SelectLabel = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => <div ref={ref} className={cn("px-2 py-1.5 text-[11px] text-white/50", className)} {...props} />
);
SelectLabel.displayName = "SelectLabel";

const SelectSeparator = forwardRef<HTMLDivElement, HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => <div ref={ref} role="separator" className={cn("my-1 -mx-1 h-px bg-white/10", className)} {...props} />
);
